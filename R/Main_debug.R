...
# test (1) by replicating the successful 10 rows to make bigger data sets. 

output_leo_t1_df_subset = output_leo_t1_df[1:10,]
output_leo_t1_df_subset_mult = output_leo_t1_df_subset

# Works for 500 rows; fails at 1000 rows due to 'query complexity'.  
for(i in 1:49){
  output_leo_t1_df_subset_mult = rbind(output_leo_t1_df_subset_mult, output_leo_t1_df_subset)
}
...
DatabaseConnector::insertTable(connection = connection,
                               databaseSchema = target_database_schema,
                               tableName=nlp_table_leo_output_t1,
                               data=output_leo_t1_df_subset_mult,
                               dropTableIfExists = TRUE,
                               createTable = TRUE,
                               tempTable = FALSE,
                               oracleTempSchema = NULL,
...
# test table appending method. 

# upload second table to GCP
output_leo_t1_df_subset = output_leo_t1_df[1:10,]
connection <- DatabaseConnector::connect(connectionDetails = connectionDetails)
DatabaseConnector::insertTable(connection = connection,
                               databaseSchema = target_database_schema,
                               tableName="leo_output_t1_temp",
                               data=output_leo_t1_df_subset,
                               dropTableIfExists = TRUE,
                               createTable = TRUE,
                               tempTable = FALSE,
...
targetTableName <- nlp_table_leo_output_t1
tempTableName <- "leo_output_t1_temp"

union_sql <- paste0("SELECT * FROM `",target_database_schema,".",tempTableName,"` UNION ALL SELECT * FROM `",target_database_schema,".",targetTableName,"`")

create_union_sql <- paste0("CREATE OR REPLACE `",target_database_schema,".",targetTableName,"` AS WITH
                           SELECT * FROM `",target_database_schema,".",tempTableName,"` UNION ALL SELECT * FROM `",target_database_schema,".",targetTableName,"`")



insert_sql <-paste0("INSERT `",target_database_schema,".",targetTableName,"` SELECT * FROM `",target_database_schema,".",tempTableName,"`")


connection <- DatabaseConnector::connect(connectionDetails = connectionDetails)
DatabaseConnector::renderTranslateExecuteSql(connection = connection,
                                             sql = create_union_sql)
DatabaseConnector::disconnect(connection)
